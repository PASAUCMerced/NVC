...
#  All Rights Reserved
#
# $Id: vars_common.pl 1863 2012-10-01 06:11:11Z CloyceS $
...
$::suitebase = 'OMP';
$::year = '2012';
$::release_year = 2012;   # Or just set to $::year
$::suite = $::suitebase.$::year;
$::current_version = 1.0; # YYY - version: Adjust by hand
$::current_runspec = 1663; # YYY - revision: Adjust by hand
$::treeowner = 'keeper';    # Mostly for cleanuptree
$::treegroup = 'ompdevel';  # Mostly for cleanuptree
...
$::nonvolatile_config = {
    ...
    'min_report_runs' => ($::lcsuite =~ /^(mpi2007|omp2001)$/) ? 2 : 3,
    # Minimum allowed temperature in C
    'min_temp_limit'  => ($::lcsuite eq 'omp2012') ? 20 : 21, # 0 => no limit
    # Maximum allowed humidity in rel%
    'max_hum_limit'   => 0,     # 0 => no limit
    # Number of power samples to discard from each end before averaging
    'discard_power_samples' => ($::lcsuite =~ /^(omp2012)$/) ? 1 : 0,
    'orig_argv'  => [ @ARGV ],
    'orig_env'   =>  { %ENV },
    'help'       => 0,
    'version'    => 0,
    ...
    'meter_errors_default' => 0.1, # Acceptable percentage of bad samples
    ...
    'max_average_uncertainty' => 1, # Maximum avg. % uncertainty of all samples
                                    # (this is hardcoded in PTD; we can't change it)
    'uncertainty_exception' => 5, # % samples allowed to exceed max_average_uncertainty
};
...
$::default_config = {
    ...
    'check_md5'           => 0,               # check saved md5 sums
    ...
    'strict_rundir_verify'=> 0,  # Make sure that the MD5s of file in the
    ...
    'meter_connect_timeout' => 30, # How long to wait before giving up
...
%fm::bmarklist = ('gross' => { 'name' => 'gross',
                             ...
                             'test' => 'test',
                             'benchmarklist' => [
                             # OMP2012 OMPG2012 benchset benchmark list begins here
                                 [ '350.md',            'F'   ],
                                 [ '351.bwaves',        'F'   ],
                                 [ '352.nab',           'C'   ],
                                 [ '357.bt331',         'F'   ],
