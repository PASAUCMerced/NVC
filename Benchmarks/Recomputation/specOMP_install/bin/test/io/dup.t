...
$echo = qq{specperl -le "print q(ok %d)"};
...
if ($^O =~ /MSWin/) {
  print "not ok 5; # TODO I have broken specperl in this regard\n";
} else {
  print `$cmd`;
}
...
if( $^O eq 'VMS' ) {
    ...
    print "not ok $_ # TODO I have broken specperl in this regard\n"
      for 6..7;
}
else {
    system sprintf $echo, 6;
    if ($^O eq 'MacOS' || $^O =~ /MSWin/) {
