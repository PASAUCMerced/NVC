...
#  All Rights Reserved
#
# $Id: log_common.pl 1198 2011-09-20 21:25:53Z CloyceS $
...
sub Log {
	...
	if (reftype($data) eq 'ARRAY') {## arrays referenced are joined w/o separator
                                        ## and then appended to the output
	    $data = join("", @{$data});
	} elsif (reftype($data) eq 'SCALAR') { ## scalar references are just appended
	    $data = ${$data};
	}
	if ($level >= 100) {
	    $level   -= 100;
...
sub split_line {
  ...
  if (exists($::partial_lines{$dest}) && (reftype($::partial_lines{$dest}) eq 'ARRAY')
