...
#
# $Id: flags.pl 1395 2011-11-23 16:48:00Z CloyceS $
...
my $flags_version = '$LastChangedRevision: 1395 $ '; # Make emacs happier
  ...
  foreach my $class (qw(forbidden unknown)) {
        ...
        for my $bench (sort keys %{$classref}) {
            ...
            foreach my $flag (@{$classref->{$bench}->{$tune}}) {
                if ($class eq 'unknown') {
                    ...
                    push @afters, @{$afters} if (reftype($afters) eq 'ARRAY');
                }
            }
            push @output, '</ul>';
	  }
    ...
    if (@afters) {
      ...
      while (@afters) {
        ...
        next unless (reftype($incref) eq 'ARRAY');
        my ($flag, $text) = @{$incref};
...
sub dump_single_flag {
    ...
    my $flag;
    if (exists($opts{'flagref'}) && (reftype($opts{'flagref'}) eq 'HASH')) {
      $flag = $opts{'flagref'};
    } else {
      $flag = $flags->{'flagmap'}->{$flagname};
    }
    return ([], '') unless (reftype($flag) eq 'HASH');
    ...
    if ($opts{'do_replacements'}
        && (::reftype($flag->{'ex_replacement'}) eq 'ARRAY')
        && @{$flag->{'ex_replacement'}}) {
...
sub flag_link_tree {
    ...
    my @output;
    $opts{'seen'} = {} unless (reftype($opts{'seen'}) eq 'HASH');

    return () unless exists($flags->{'flagmap'}->{$flagname}) && (reftype($flags->{'flagmap'}->{$flagname}) eq 'HASH');
              ...
              if ($text =~ /(?!<=\\)\$\d/) {
                  ...
                  if (@replacements == 0) {
                      if ($link_text eq '' && $opts{'do_replacements'}) {
                          if (   (::reftype($flag->{'ex_replacement'}) eq 'ARRAY')
                              && @{$flag->{'ex_replacement'}}) {
                              @replacements = @{$flag->{'ex_replacement'}};
                          }
                      }
