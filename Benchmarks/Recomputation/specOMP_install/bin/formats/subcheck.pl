...
#
# $Id: subcheck.pl 1727 2012-07-24 19:42:08Z CloyceS $
...
my $subcheck_version = '$LastChangedRevision: 1727 $ '; # Make emacs happier
...
sub format {
    ...
    my @syntax_types = ();
    my %syntax_lists = ();
    my @written = (::reftype($written) eq 'ARRAY') ? @{$written} : ( $written );
    ...
    if ($::lcsuite eq 'mpi2007') {
        ...
        foreach my $item (qw(node interconnect)) {
            ...
            foreach my $system (sort {
                ...
                my @subs = grep { (::reftype($syntax{$item}->{$_}) eq 'HASH') && !exists($syntax{$item}->{$_}->{'regexp'}) } keys %{$syntax{$item}};
	...
	my @lines = grep(/^\Q$prefix$syntax_type\E$key[\s=]/, @{$data_ref});
        ...
        if ((::reftype($syntax_ref->{$key}) eq 'ARRAY')) {
          ...
          $explanation = [ $explanation ] unless (::reftype($explanation) eq 'ARRAY');
          $default = 0;
        } elsif ( (::reftype($syntax_ref->{$key}) eq 'HASH') ) {
        ...
        if (defined($conditions) && (::reftype($conditions) eq 'ARRAY')) {
            my $all_ok = 1;
            my @conditions = @{$conditions};
            if ((::reftype($conditions[0]) ne 'ARRAY')) {
            ...
            foreach my $constraintref (@conditions) {
                ...
                foreach my $constraint (@constraints) {
                    ...
                    if ((::reftype($constraint) eq 'ARRAY')) {
                        ($min, $max) = @{$constraint};
                    } else {
                        $regex = qr/$constraint/;
                    }
