...
#            Cloyce D. Spradling
#
# $Id: raw.pl 1841 2012-09-04 20:48:23Z CloyceS $
...
my $version = '$LastChangedRevision: 1841 $ '; # Make emacs happier
...
sub format {
    ...
    foreach my $system ('', keys %{$r->{'mpi_items'}}) {
        foreach my $section (map { $_->[0] } @::notes_info) {
            next unless (::reftype($r->{$system.$section}) eq 'HASH');
            ...
            foreach my $key (sort keys %{$r->{$system.$section}}) {
                ...
                next unless (::reftype($val) eq 'ARRAY');
                for (my $i = 0; $i < @$val; $i++) {
                    my ($tag, $text) = @{$val->[$i]};
    ...
    if ($r->{'invalid'} == 0) {
        ...
        for my $bench (sort keys %{$r->{'results'}}) {
            for my $tune (keys %{$r->{'results'}{$bench}}) {
                next unless (::reftype($r->{'results'}{$bench}{$tune}{'data'}) eq 'ARRAY');
    ...
    # CVT2DEV: || 1
        ...
        if (!exists($r->{'errors'}) ||
            (exists($r->{'errors'}) && (::reftype($r->{'errors'}) eq 'ARRAY') &&
	...
	my $val = $r->accessor($key);
        ...
        if ($key eq 'flagsurl') {
            ...
            if (   ((::reftype($val) eq 'ARRAY') && @$val >= 1)
                || ((::reftype($val) ne 'ARRAY') && $val ne '')) {
                if ((::reftype($val) eq 'ARRAY') && @$val > 1) {
        ...
        # For power/temp-related raw records, squeeze them down
            ...
            $val = squeeze_power_samples($key, $val);
        }
    ...
    if (!exists($r->{'exehash'})) {
	Log(0, "\nERROR: No saved executable hashes in the result object!\n");
	main::do_exit(1);
    }
	...
	for my $tune (keys %{$r->{'results'}{$bench}}) {
	    ...
	    # Dump the executable hashes for future inspection
	    if (exists($r->{'exehash'}->{$bench}) &&
		exists($r->{'exehash'}->{$bench}->{$tune})) {
                push (@md5list, 
                    sprintf "%s.exehash.%s.%s: %s", 
                            $prefix, $benchname, $tune,
                            $r->{'exehash'}->{$bench}->{$tune});
	    }
    ...
    if ($r->{'unknownused'}) {
      if (!exists $r->{'errors'} || (::reftype($r->{'errors'}) ne 'ARRAY') ||
          @{$r->{'errors'}} == 0) {
...
sub squeeze_power_samples {
    my ($key, $val) = @_;
    my $tmp = $val;

    if (ref($tmp) ne '') {
        my $dumper = Data::Dumper->new([$val],[$key]);
        $dumper->Indent(0);
        $dumper->Purity(1);
        $dumper->Terse(1);
        $tmp = $dumper->Dump;
    }
    my ($raw, $comp, $enc) = ::compress_encode($tmp);
    return [ split(/\n/, (defined($enc) && $enc ne '') ? $enc : $raw) ];
}
