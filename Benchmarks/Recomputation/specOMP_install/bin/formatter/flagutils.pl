...
#  All Rights Reserved
#
# $Id: flagutils.pl 1737 2012-07-27 13:49:51Z CloyceS $
...
my $version = '$LastChangedRevision: 1737 $ '; # Make emacs happier
...
sub flags_list {
    ...
    return undef unless (reftype($config->{'flaginfo'}) eq 'HASH');
    ...
    $s->share_from('main', [ 'check_elem', 'reftype', 'Log', 'istrue' ]);
...
#-----------------------------------------------------------------------------
    ...
    sub adjust_flag_pos {
      ...
      foreach my $class (@{$classlist}) {
        next unless (exists($flaglist->{$class}) && (reftype($flaglist->{$class}) eq 'ARRAY'));
              ...
              for(my $i = 0; $i <= $#{$opts}; $i++) {
                ...
                while (defined($re) && defined($flag)) {
                    ...
                    if (exists($currflag->{'inc_text'})
                        && (reftype($currflag->{'inc_text'}) eq 'ARRAY')) {
                    ...
                    if (exists($flagdesc_copy->{'inc_flag'})
                        && (reftype($flagdesc_copy->{'inc_flag'}) eq 'ARRAY')) {
                    ...
                    if ($var eq 'LD' &&
                        (reftype($flaglist->{'optimization'}->[0]) eq 'ARRAY') &&
...
%);
    ...
    foreach my $class (@classlist) {
      next unless (reftype($flaglist->{$class}) eq 'ARRAY');
      my $i = 0;
    ...
    foreach my $class (@classlist) {
      ...
      if (reftype($flaglist->{$class}) eq 'ARRAY') {
...
sub reduce_flags {
	...
	next unless defined($r->{'results'}->{$bench}) && (reftype($r->{'results'}->{$bench}) eq 'HASH');
	my $benchref = $r->{'results'}->{$bench};
	foreach my $tune (keys %{$benchref}) {
	    next unless defined($benchref->{$tune}) && (reftype($benchref->{$tune}) eq 'HASH');
	    my $tuneref = $benchref->{$tune};
	    next unless exists($tuneref->{'flags'}) && (reftype($tuneref->{'flags'}) eq 'HASH');
	    ...
	    if (reftype($rc->{'compilers'}->{$bench}->{$tune}) eq 'ARRAY') {
    ...
    if ($r->{'basepeak'} && grep { $_ eq 'peak' } @{$r->{'tunelist'}}) {
      ...
      if ($r->{'basepeak'} == 1) {
        ...
        for my $bench (keys %{$r->{'results'}}) {
          ...
          next unless (reftype($r->{'results'}{$bench}{'peak'}{'data'}->[0]) eq 'HASH');
      ...
      foreach my $tune (keys %{$rc->{'tunelist'}}) {
        ...
        foreach my $class (keys %{$rc->{'classlist'}}) {
          ...
          for(my $i = 0; $i < @{$flaglistref}; $i++) {
            ...
            next unless (reftype($flag) eq 'ARRAY');
            if ($class =~ /(?:forbidden|unknown)/) {
              ...
              next unless (reftype($flag->[2]) eq 'HASH');
      ...
      if (::check_elem('HASH', $rc, 'bylang', 'flaglist', $class)) {
        foreach my $tune (keys %{$rc->{'bylang'}->{'flaglist'}->{$class}}) {
          ...
          foreach my $lang (keys %{$tuneref}) {
            next unless (reftype($tuneref->{$lang}) eq 'ARRAY');
            # Build a hash with counts of the flag text
            my %counts = ();
            map { $counts{$_->[1]}++ } grep { (reftype($_) eq 'ARRAY') } @{$tuneref->{$lang}};
      ...
      if (::check_elem('HASH', $rc, 'flaglist', $class)) {
        foreach my $bench (keys %{$rc->{'flaglist'}->{$class}}) {
          ...
          foreach my $tune (keys %{$rc->{'flaglist'}->{$class}->{$bench}}) {
            ...
            # Build a hash with counts of the flag text
            my %counts = ();
            map { $counts{$_->[1]}++ } grep { (reftype($_) eq 'ARRAY') } @{$tuneref};
            # Weed out the ones that aren't duplicates
            map { delete $counts{$_} } grep { $counts{$_} <= 1 } keys %counts;
            foreach my $dupflag (keys %counts) {
...
sub munge_markup {
  ...
  for(my $i = 0; $i < @{$flaglist}; $i++) {
    next unless (reftype($flaglist->[$i]) eq 'ARRAY');
...
sub find_flags_refs {
    ...
    $seen = {} unless (reftype($seen) eq 'HASH');
	...
	if (reftype($flaghash->{$flagname}) ne 'HASH') {
...
sub markup_flag {
  ...
  $passes = [ $passes ] if (reftype($passes) ne 'ARRAY');
  my @classes = grep { defined } @{$opts{'classes'}} if (reftype($opts{'classes'}) eq 'ARRAY');
  ...
  return ($mismatch, $flag->[1].$markup, $markup) if (reftype($passes) ne 'ARRAY');
  ...
  foreach my $passref (@{$flag}[3..$#{$flag}]) {
    next if (reftype($passref) ne 'ARRAY');
...
sub check_same_flags {
  ...
  return 0 unless (reftype($lines) eq 'HASH');
  ...
  while ($same && @items) {
    my $item = shift(@items);
    if (reftype($lines->{$item}) ne 'ARRAY') {
