...
void genmat (double *M[])
{
   ...
   for (ii=0; ii < bots_arg_size; ii++)
   {
	 ...
	 if (ii==jj-1) null_entry = FALSE;
	   ...
	   M[ii*bots_arg_size+jj] = (double*)tmp + tmp_pos;
...
void sparselu_init (double ***pBENCH, char *pass)
{
  ...
  // printf("%s\n", pass):
   tmp = malloc(bots_arg_size*bots_arg_size*sizeof(double *) + bots_arg_size * ((bots_arg_size*bots_arg_size)*bots_arg_size_1*bots_arg_size_1*sizeof(double)));
  *pBENCH = (double **)tmp + tmp_pos;
   tmp_pos += bots_arg_size*bots_arg_size*sizeof(double *);

   genmat(*pBENCH);

	crucial_data(tmp, "double", bots_arg_size*bots_arg_size + (3)*(bots_arg_size*bots_arg_size*bots_arg_size_1*bots_arg_size_1));
