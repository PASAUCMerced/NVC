...
void genmat (float *M[])
{
   ...
   for (ii=0; ii < bots_arg_size; ii++)
   {
	 ...
	 if (ii==jj-1) null_entry = FALSE;
	   ...
	   M[ii*bots_arg_size+jj] = (float*)tmp + tmp_pos;
...
void sparselu_init (float ***pBENCH, char *pass)
{
  ...
  // printf("%s\n", pass):
   tmp = malloc(bots_arg_size*bots_arg_size*sizeof(float *) + (3)*(bots_arg_size*bots_arg_size*bots_arg_size_1*bots_arg_size_1*sizeof(float)));
  *pBENCH = (float **)tmp + tmp_pos;
   tmp_pos += bots_arg_size*bots_arg_size*sizeof(float *);

   genmat(*pBENCH);

	crucial_data(tmp, "double", bots_arg_size*bots_arg_size*sizeof(float *) + (bots_arg_size*bots_arg_size*bots_arg_size_1*bots_arg_size_1*sizeof(float)));
