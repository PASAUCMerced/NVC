...
INT_TYPE key_array[SIZE_OF_BUFFERS],
         key_array_tmp[SIZE_OF_BUFFERS],
         key_buff1[MAX_KEY],
         key_buff1_tmp[MAX_KEY],
         key_buff2[SIZE_OF_BUFFERS],
         key_buff2_tmp[SIZE_OF_BUFFERS],
         //key_buff2_tmp[SIZE_OF_BUFFERS],
         partial_verify_vals[TEST_ARRAY_SIZE],
         partial_verify_vals_tmp[TEST_ARRAY_SIZE],
         **key_buff1_aptr = NULL;

#ifdef USE_BUCKETS
//INT_TYPE **bucket_size,
INT_TYPE    bucket_ptrs[NUM_BUCKETS];
INT_TYPE    bucket_ptrs_tmp[NUM_BUCKETS];
        //bucket_size[i] = (INT_TYPE *)alloc_mem(sizeof(INT_TYPE) * NUM_BUCKETS);
INT_TYPE bucket_size[1][NUM_BUCKETS];
INT_TYPE bucket_size_tmp[1][NUM_BUCKETS];
...
/*  Accumulative bucket sizes are the bucket pointers.
  ...
  //  if(iteration == 2) printf("max flag3 = %d\n",myid);
    ...
    if(recompute == 0) flag4=0;
    //if(iteration == 2) printf("max flag4 = %d\n",NUM_BUCKETS);
    for( i=flag4+1; i< NUM_BUCKETS; i++ ) {
...
//}
    if(recompute == 0) flag5 = -1;
    //if(iteration == 2) printf("max flag5 = %d\n",NUM_KEYS);
/*  Sort into appropriate bucket */
    #pragma omp for schedule(static)
    for( i=flag5+1; i<NUM_KEYS; i++ )
    {
        k = key_array[i];
...
//if(iteration != temp_iteration)
  if(recompute == 0)  flag7 = -1;
#ifdef SCHED_CYCLIC
    #pragma omp for schedule(static,1)
#else
    #pragma omp for schedule(dynamic)
#endif
    for( i=flag7+1; i< NUM_BUCKETS; i++ ) {
...
/*  In this section, the keys themselves are used as their
    ...
    individual population                                       */
        ...
        //if(recompute == 0)
            flag1 = m-1;
        for ( k = flag1+1; k < bucket_ptrs[i]; k++ ) {
...
/*  Start timer  */
  ...
  //flush_whole_cache();
  //start_crash();
    addr[count_addr++] = key_array;
    addr[count_addr++] = key_buff1;
    addr[count_addr++] = key_buff2;
    addr[count_addr++] = partial_verify_vals_tmp;
    addr[count_addr++] = bucket_ptrs;
    addr[count_addr++] = bucket_size_tmp;
    addr[count_addr++] = &flag1;
    addr[count_addr++] = &flag2;
    addr[count_addr++] = &flag3;
    addr[count_addr++] = &flag4;
    addr[count_addr++] = &flag5;
    addr[count_addr++] = &flag6;
    addr[count_addr++] = &flag7;
    addr[count_addr++] = &flag8;
    addr[count_addr++] = &temp_iteration;
...
/*  This tests that keys are in sequence: sorting of last ranked key seq
    ...
    if (timer_on) timer_stop( 3 );
     ...
     passed_verification+=(temp_iteration-1)*5;
/*  The final printout  */
