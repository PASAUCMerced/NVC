...
#            Cloyce D. Spradling
#
# $Id: setup_common.pl 1198 2011-09-20 21:25:53Z CloyceS $

my $tmpversion = '$LastChangedRevision: 1198 $ '; # Make emacs happier
...
sub read_manifests {
    ...
    foreach my $file (@files) {
        my ($re, $fn) = ((reftype($file) eq 'ARRAY') ? @{$file} : (qr/./, $file));
