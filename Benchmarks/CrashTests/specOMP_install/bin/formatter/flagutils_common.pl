...
#  All Rights Reserved
#
# $Id: flagutils_common.pl 1727 2012-07-24 19:42:08Z CloyceS $
...
sub update_flags_file {
    ...
    if ($bm ne $::lcsuite && $bm !~ /(?:\.syntax|^dist-.*\.(?:cfg|xml))$/) {
      ...
      if (ref($config->{'benchmarks'}->{$bm}) ne "Spec::Benchmark::$bmname$bmnum") {
    ...
    foreach my $flags (@files) {
        ...
        if (defined($flagparser)) {
            ...
            if (reftype($flagref) ne 'HASH') {
                ...
                if (exists($opts{'forbid'}) && (reftype($opts{'forbid'}) eq 'ARRAY') &&
                    @{$opts{'forbid'}}) {
                ...
                foreach my $flag (@{$flags}) {
                    ...
                    if (exists($flag->{'inc_flag'}) && reftype($flag->{'inc_flag'}) eq 'ARRAY') {
                        # Check that the referenced flags have descriptions
                        foreach my $incflagref (@{$flag->{'inc_flag'}}) {
                            if (reftype($incflagref) ne 'ARRAY') {
                              Log(0, "\nERROR: While parsing flags file");
                              Log(0, " at $url") if ($url ne '');
                              Log(0, ":\n");
...
sub make_flag_id {
  ...
  return undef unless (reftype($flag) eq 'HASH');
  my $sum = '';
  ...
  if (exists($flag->{'idcache'}) && (reftype($flag->{'idcache'}) eq 'HASH') &&
...
sub validate_xml {
    ...
    my $top;
    if (defined($::global_config) && (reftype($::global_config) eq 'HASH')) {
    ...
    if ($suitever eq 'select') {
        ...
        $dtd = ::jp($top, 'Docs', 'flags', $dtd);
             ...
             || ($::lcsuite eq 'cpu2006' && (($suitever >= 1 && $suitever <= 1.1) || ($suitever >  4 && $suitever <= 113)))
            ) {
        # Use the old DTD for MPI2007 and CPU2006 v1.1 and earlier
