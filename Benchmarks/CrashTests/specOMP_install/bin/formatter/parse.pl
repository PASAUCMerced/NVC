...
#  All Rights Reserved
#
# $Id: parse.pl 1890 2012-10-10 19:17:53Z CloyceS $
...
my $version = '$LastChangedRevision: 1890 $ '; # Make emacs happier
...
sub parse_commandline {
    ...
    if ($cl_opts->{'nopower'}) {
      $config->{'nopower'} = $cl_opts->{'nopower'};
    } else {
      $config->{'nopower'} = 0;
    }
    if ($cl_opts->{'nopeak'}) {
      $config->{'nopeak'} = $cl_opts->{'nopeak'};
    } else {
      $config->{'nopeak'} = 0;
    }
...
sub parse_raw {
	...
	if ($tag !~ /\./o) { # 'info' thing
                    ...
                    foreach my $notere (@::notes_regexps) {
                        if ($tag =~ /$notere/) {
                            ...
                            $r->{$section} = {} unless (exists($r->{$section})
                                            && (reftype($r->{$section}) eq 'HASH'));
                            $r->{$section}->{$key} = [] unless (exists($r->{$section}->{$key})
                                            && (reftype($r->{$section}->{$key}) eq 'ARRAY'));
	    ...
	    my @comps = split(/\./, $tag);
            ...
            } elsif ($what =~ /^exehash$/o) {
                my ($bench, $tune) = @comps;
                $bench =~ s/_/./;
                $exehash->{$bench}->{$tune} = $value;
            } else {
    ...
    if (istrue($r->{'power'}) && !istrue($config->{'nopower'})) {
        ...
        foreach my $key (keys %{$r}) {
            delete $r->{$key} if $key =~ /^(?:power|temperature|hw_power|hw_temperature)_/;
            delete $r->{$key} if $key =~ /^(?:idle_)?(?:max|min|avg)_(?:hum|power|temp|uncertainty)$/;
            delete $r->{$key} if $key =~ /^(?:idle_)?(?:amp|volt)_range\d*$/;
        }
            ...
            foreach my $res (@{$tref}) {
                foreach my $thing (qw(power temp uncertainty hum)) {
                    if (istrue($r->{'power'}) && !istrue($config->{'nopower'})) {
                        ...
                        my (undef, $decode, $decomp) = decode_decompress($lines);
                ...
                if (istrue($r->{'power'}) && !istrue($config->{'nopower'})) {
                    my %tmpranges = ::extract_ranges($res, $res->{'powersamples'}, '', @{$r->{'meterlist'}->{'power'}});
                    if (%tmpranges) {
    ...
    if (istrue($r->{'power'}) && !istrue($config->{'nopower'})) {
        my %tmpranges = ::extract_ranges($r, $r->{'idle_powersamples'}, 'idle_', @{$r->{'meterlist'}->{'power'}});
        if (%tmpranges) {
            # Update the hash of ranges used per-meter
            foreach my $metertag (keys %tmpranges) {
                $r->{'meterranges'}->{$metertag}->{'amp'}->{$tmpranges{$metertag}->{'amp'}}++;
    ...
    if (istrue($r->{'power'}) && !istrue($config->{'nopower'})) {
        foreach my $metertag (keys %{$r->{'meterranges'}}) {
            $r->{'power_'.$metertag.'_current_range'} = join('A, ', sort { $a <=> $b } keys %{$r->{'meterranges'}->{$metertag}->{'amp'}}).'A';
            $::generated_fields{'power_'.$metertag.'_current_range'}++;
            $r->{'power_'.$metertag.'_voltage_range'} = join('V, ', sort { $a <=> $b } keys %{$r->{'meterranges'}->{$metertag}->{'volt'}}).'V';
    ...
    if ($ignoreflags) {
        foreach my $flagsurl (grep { defined($_) && $_ ne '' } @{$flagsurls}) {
            if ($flagsurl eq 'noflags') {
                ...
                $r->{'flaginfo'}->{'user'} = {} unless (reftype($r->{'flaginfo'}->{'user'}) eq 'HASH');
	...
	my $parsed = 0;
        if (exists $r->{'flagsurl'} && (reftype($r->{'flagsurl'}) ne 'ARRAY')) {
    ...
    # Put the compile options and executable hashes back into the result object
    ...
    foreach my $bench (keys %{$exehash}) {
        next unless ref($exehash->{$bench}) eq 'HASH';
        foreach my $tune (keys %{$exehash->{$bench}}) {
            $r->{'exehash'}->{$bench}->{$tune} = $exehash->{$bench}->{$tune};
        }
    }
    ...
    foreach my $section (grep { /^$::mpi_desc_re_id?notes/ } keys %{$r}) {
        next unless (reftype($r->{$section}) eq 'HASH');
        foreach my $key (keys %{$r->{$section}}) {
            next unless (reftype($r->{$section}->{$key}) eq 'ARRAY');
    ...
    if ($r->{'submit'}) {
        $r->{'notes_submit'} = {} unless (::reftype($r->{'notes_submit'}) eq 'HASH');
        my $fullnotes = '';
        my $notes = $r->{'notes_submit'};
        foreach my $key (sort keys %{$r->{'notes_submit'}}) {
            next unless (::reftype($notes->{$key}) eq 'ARRAY');
            my $val = $notes->{$key};
            for (my $i = 0; $i < @$val; $i++) {
                $fullnotes .= ' '.$val->[$i]->[1];
            }
        ...
        # Get rid of them if present; bug in CPU2006 v1.2
        delete $r->{'notes_submit'};
    }
    ...
    if (defined($::website_formatter) && $::website_formatter) {
      ...
      foreach my $cfg (qw(txtconfig orig_raw_config)) {
        next unless (exists($r->{$cfg}) && (reftype($r->{$cfg}) eq 'ARRAY'));
