...
//kai
   crucial_data(m_regNumList, "int", numElem());
   // Elem-centered
...
//kai
	crucial_data(&m_numReg, "int", 1);
 	crucial_data(&m_cost, "int", 1);
	crucial_data(&m_dtcourant, "double", 1);
        crucial_data(&m_dthydro, "double", 1);
        crucial_data(&m_cycle, "int", 1);
        crucial_data(&m_dtfixed, "double", 1);
        crucial_data(&m_time, "double", 1);
        crucial_data(&m_deltatime, "double", 1);
        crucial_data(&m_deltatimemultlb, "double", 1);
        crucial_data(&m_deltatimemultub, "double", 1);
        crucial_data(&m_dtmax, "double", 1);
        crucial_data(&m_stoptime, "double", 1);

        crucial_data(&m_numRanks, "int", 1);
        crucial_data(&m_colLoc, "int", 1);
        crucial_data(&m_rowLoc, "int", 1);
        crucial_data(&m_planeLoc, "int", 1);
        crucial_data(&m_tp, "int", 1);
        crucial_data(&m_sizeX, "int", 1);
        crucial_data(&m_sizeY, "int", 1);
        crucial_data(&m_sizeZ, "int", 1);
        crucial_data(&m_numElem, "int", 1);
        crucial_data(&m_numNode, "int", 1);
        crucial_data(&m_maxPlaneSize, "int", 1);
        crucial_data(&m_maxEdgeSize, "int", 1);
	crucial_data(&m_rowMin, "int", 1);
	crucial_data(&m_rowMax, "int", 1);
	crucial_data(&m_colMin, "int", 1);
	crucial_data(&m_colMax, "int", 1);
        crucial_data(&m_planeMin, "int", 1);
        crucial_data(&m_planeMax, "int", 1);
...
//kai
//	crucial_data(nodeElemCount, "int", numNode);
    for (Index_t i=0; i<numNode(); ++i) {
      nodeElemCount[i] = 0 ;
    }
...
//kai
        crucial_data(m_nodeElemStart, "int", numNode()+1);
...
//kai
        crucial_data(m_nodeElemCornerList, "int", m_nodeElemStart[numNode()]);
...
//kai
	crucial_data(&m_symmX, "int", edgeNodes*edgeNodes);
  printf("edgeNode is %d\n",edgeNodes*edgeNodes);
   }
  if (m_rowLoc == 0) {
    m_symmY.resize(edgeNodes*edgeNodes);
	//kai
	 crucial_data(&m_symmY, "int", edgeNodes*edgeNodes);
	}
  if (m_planeLoc == 0) {
    m_symmZ.resize(edgeNodes*edgeNodes);
	//kai
	 crucial_data(&m_symmZ, "int", edgeNodes*edgeNodes);
   }
}
...
//kai
	crucial_data(m_regElemSize, "int", numReg());
	crucial_data(m_regElemlist, "int", numReg());
   Index_t nextIndex = 0;
   ...
   for (Index_t i=0 ; i<numReg() ; ++i) {
      ...
      regElemSize(i) = 0;
//kai
	crucial_data(m_regElemlist[i], "int", regElemSize(i));
   }
