...
#            Cloyce D. Spradling
#
# $Id: csv.pl 1727 2012-07-24 19:42:08Z CloyceS $
...
my $csv_version = '$LastChangedRevision: 1727 $ '; # Make emacs happier
...
sub format {
    ...
    if ($::lcsuite eq 'mpi2007') {
      ...
      if (istrue($r->rate)) {
        $bmode = '# Copies';
      } elsif ($::lcsuite eq 'cpuv6' || $::lcsuite eq 'omp2012') {
        $bmode = '# Threads';
      } else {
        $bmode = 'Ref Time';
      }
    ...
    for my $bench (sort keys %{$r->benchmarks}) {
        ...
        for(my $i = 0; $i < @tunelist; $i++) {
            if (@nc > 0 && ::check_elem('ARRAY', $r, 'results', $bench, $tunelist[$i], 'data')) {
                ...
                (reftype($selected{$bench}->{$tunelist[$i]}) ne 'ARRAY')) {
