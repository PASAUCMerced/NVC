...
#            Cloyce D. Spradling
#
# $Id: ps.pl 1890 2012-10-10 19:17:53Z CloyceS $
...
my $ps_version = '$LastChangedRevision: 1890 $ '; # Make emacs happier
...
sub format () {
    ...
    return undef if ::istrue($r->power);        # Temporarily unimplemented
...
#print "\nSPEC_info_box(p=$p, x=$x, y=$y, width=$width, height=$height, config=$config, list=$list, size=$size, section=\"$section\")\n";
	...
	if ($reftype eq 'ARRAY') {
	    ...
	    my $mysize = $size;
	    for my $subval (@vals) {
                next if (::reftype($subval) eq 'HASH'); # Ignore destinations
                my $testtxt = '';
...
#    $width -= 1;		# Border space
	...
	my %results = (
        ...
        foreach my $res (@results_list) {
            ...
            if (::istrue($config->rate) || $::lcsuite eq 'cpu2006') {
	...
	my $rate_x = 0;
        my $rtext_pad = 0;
        my $copies_size = $size;
	if ($::lcsuite ne 'cpu2006') { # All benchmarks have ranks/threads/etc
	    # Make space for the copies label
            my $long_copies_str = ('0') x length($maxcopies);
	    my @copies_width = ($p->stringwidth('Threads', $copies_size) + ($border * 2),
	...
	if ($rate_x) {
	    ...
	    $p->set_text_pos($graph_x - ($rate_x / 2), $string_base);
            if ($::lcsuite eq 'mpi2007') {
                $p->show_center('Ranks');
            } elsif ($::lcsuite eq 'omp2012' || $::lcsuite eq 'cpuv6') {
                $p->show_center('Threads');
            } else {
...
sub do_bm_graph {
    ...
    my ($p, $y, $geom, $what, $results, $avoid, $config) = @_;

    #print "\ndo_bm_graph(p=$p, y=$y, geom=$geom, what=$what, results=$results, avoid=$avoid, config=$config)\n";
...
sub SPEC_table {
	...
	for my $bench (sort keys %{$config->benchmarks}) {
		...
		if ($rate) {
                    my $copies;
                    if ($rate == 2 || $::lcsuite =~ /^(mpi2007|omp2012)$/) {
		...
		if (grep { /^${tune}$/ } @{$config->tunelist}) {
			...
			next unless (reftype($results[$idx]) eq 'HASH');
			my $res = $results[$idx];
			if ($is_nc) {
...
sub SPEC_mpi_detail {
    ...
    for my $item (qw(node interconnect)) {
        next unless exists($config->{$item}) && (::reftype($config->{$item}) eq 'HASH');
        my $iref = $config->{$item};
    ...
    for(my $i = 0; $i < @lines; $i++) {
      ...
      if ($subsection_title) {
        ...
        $required_height *= 3;
          ...
          my $tmptext = '';
          if ($i == @lines-1) {
              $tmptext = (reftype($text) eq 'ARRAY') ? join('', @$text) : $text;
...
sub SPEC_notes {
  my ($p, $config, $width, $height, $did_continue, $notes, $systype) = @_;
    ...
    return (0, 0, 0) unless (::reftype($notes) eq 'ARRAY');
...
sub munge_links {
  ...
  foreach my $lineref (@lines) {
      $lineref = [ $lineref ] unless (::reftype($lineref) eq 'ARRAY');
      for(my $i = 0; $i < @{$lineref}; $i++) {
          my $chunk = $lineref->[$i];
          $chunk = [ undef, $chunk ] unless (::reftype($chunk) eq 'ARRAY');
          my ($url, $txt) = @{$chunk};
          my @newchunks = ();
          my $count = 0;
          while ($count < 40 &&
