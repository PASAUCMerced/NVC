...
#  All Rights Reserved
#
# $Id: PSPDF.pm 1198 2011-09-20 21:25:53Z CloyceS $
#
...
sub fit_link_center {
    ...
    if (reftype($str) eq 'ARRAY') {
        ...
        my $tmpstr = join('', map { (reftype($_) eq 'ARRAY') ? $_->[1] : $_ } @{$str});
...
#print "show($me, str=\"$str\", ul=$ul, url=\"$url\",opts=(\n    ".join(",\n    ", map { "$_ => $opts{$_}" } sort keys %opts)."\n   ))\n";
    ...
    if ($me->{'mode'} eq 'PDF') {
        ...
        if (defined($url) && $url ne '') {
            ...
            if ((::reftype($url) eq 'PDF::API2::NamedDestination')) {
                $ant->dest($url);
            } else {
                $ant->url($url);
            }
